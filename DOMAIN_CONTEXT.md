...
_Status_: no exchange can be a tenant yet — `ewyaix` is not in `AVALANCHE_CENTERS` and `avalancheCenterSchema` rejects their `config: null` — so every consumer of this term ships dark until #269.
