...
13. **Expose it to the MCP server with a no-tenant note.** Mark it `'find'` in `MCP_COLLECTIONS` (`src/constants/mcp.ts`), and add an exception line to the server instructions in `src/plugins/index.ts`, which otherwise tell clients to filter everything by `tenant`. See [`docs/mcp-server.md`](mcp-server.md#adding-a-collection-to-mcp).
