...
13. **Expose it to the MCP server with a no-tenant note.** Add it find-only to `mcpPlugin` in `src/plugins/index.ts`, and add an exception line to the server instructions, which otherwise tell clients to filter everything by `tenant`. See [`docs/mcp-server.md`](mcp-server.md#adding-a-collection-to-mcp).
