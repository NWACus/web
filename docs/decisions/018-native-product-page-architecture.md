...
- The E2E harness these pages are tested through adds three more `.fallowrc.json` entries, for reasons unrelated to the model: the MSW preload is a runtime root nothing imports, so it is declared under `entry`; and `scripts/e2e/**` and `__tests__/e2e/**` are excluded from `health`, because CRAP is estimated from export references and every uncovered helper in that tree therefore reads as critical regardless of its shape. See [afp-products/e2e-mocks.md](../afp-products/e2e-mocks.md).
- `react-dom` is listed under `ignoreDependencies` for the same class of reason. Next requires it at runtime but the app never imports it directly, so the moment a test did — the freshness work hydrates a component to prove the expiry banner has no hydration mismatch — dependency analysis read it as test-only and suggested moving it to `devDependencies`, which would break the build.
