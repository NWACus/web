...
- **Stids are unique within a source, not globally.** The rendering pipeline keys stations by stid, as SnowObs's own response does, and the field keeps a station once per page by `(source, stid)`. Two sources tracking the same stid would collide in a page's table. NWAC's 213 tracked stations have no such pair today; if one appears, key the table helpers by the pair.
