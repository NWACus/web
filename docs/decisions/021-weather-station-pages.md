...
- **Stids are not guaranteed unique within a center.** SnowObs namespaces them by source, and a center's tracking list spans sources, so two of a center's stations could share a stid. The rendering pipeline keys stations by stid within a center, as SnowObs's own response does, and the field keeps a station once per page by `(source, stid)`. NWAC's 213 tracked stations have no such pair today (its own loggers are small integers, SNOTEL's are four digits, Mesowest's are alphanumeric); if one appears, key the table helpers by the pair.
...
- **The registry's tests went with it.** `weatherStations.server.test.ts` (the hand-checked column lists) is replaced by `deriveColumns.server.test.ts`, `stationPages.server.test.ts` and `stationsField.server.test.ts`, which test the derivation, assembly and field validation as pure functions against fixtures, not against NWAC's 32 pages.
