...
A block rather than a per-center settings document ([ADR 016](016-per-tenant-globals-as-unique-tenant-collections.md)) because the table is content on a page: its title, intro and placement are the editor's, and a center can put two tables with different gauges on two pages if it wants. The block's component is a server component that fetches the 72-hour window with a five-minute `revalidate`, which Next takes as the page's revalidate too, so the static page regenerates on the legacy cadence rather than the hourly page default. SnowObs being unreachable renders a short notice in the block's place instead of failing the page.
