...
- **Stations are keyed by stid within a list.** SnowObs's response keys them that way, so the field refuses two sources sharing an id on one page or block, and the graph-data route refuses an id that different pages list under different sources instead of guessing. NWAC's tracked ids never overlap across sources today.
