...
Two wrinkles come with the wrapper. `@charset` and `@import` are only legal at the top of a stylesheet, so a leading run of them is hoisted back above the `@scope` block — nested inside it they are invalid and the browser drops them silently, which would leave a snippet that pulls its base CSS that way (Mailchimp's older Classic embed code does) with none at all. Their rules land unscoped, which is the trade for the embed rendering at all. And the cascade now runs the other way too: Tailwind's Preflight zeroes borders and padding on every element in the page and strips button backgrounds, which the iframe used to keep out. Sierra's and Payette's Mailchimp footers set only `border-radius` on their fields, so in the page their email inputs lose their boundary entirely; the block emits a scoped reset restoring the browser defaults for `input`, `select`, `textarea` and `button`. Element selectors beat Preflight's `*` and lose to the provider's own rules, so a snippet that does style its controls still wins — Sawtooth's declares `border: 1px solid #ccc` on its text and email fields and keeps it.

**Ordering the scripts.** A `<script>` parsed out of a string is inert, so each one is rebuilt as a fresh element. They are inserted one at a time, waiting on each non-`async` external script before inserting the next. Under the HTML parser a blocking external script also held back the scripts after it; with DOM insertion an inline script runs the instant it lands, so without the wait, Sawtooth's `jQuery.noConflict(true)` would execute before the `mc-validate.js` that defines `jQuery`. Eventbrite's embed has the same loader-plus-inline-call shape. The wait is capped at ten seconds and is also settled by the effect cleanup: a CDN that accepts the connection and never answers fires neither `load` nor `error` until the network layer gives up, and nothing after it — the inline call included — should sit behind that.
...
- The scoping covers static `<style>` elements inside the container, and nothing else. CSS a provider's loader appends to `document.head` at runtime never passes through it, so it reaches the whole page — which is what makes the Classy overlay render correctly, and equally what would let a future provider restyle the site. And because `@scope` matches only inside its root, nodes an SDK appends to `document.body` are siblings of that root rather than descendants, so a snippet's own styling for its modal will not apply to it. Shadow DOM would close neither gap and would open others: it blocks head-injected provider CSS from reaching the embed and breaks the `document.getElementById` lookups provider scripts rely on.
...
- The site's CSS now styles the embed. The scoped reset covers the form controls Preflight resets, but not everything: headings inside an embed inherit the site's typography rather than the browser's, which is the more useful default and is what Sawtooth's snippet asks for explicitly. A browser without `@scope` drops the reset along with the provider's own styling.
- A block with no embed code renders nothing at all, as `EmbedFrame` did. Payload skips required-field validation on drafts, so a `formEmbed` saved before its code is pasted is reachable in preview.
