...
The `weather` flag covers the Mountain Weather page for both sources: NWAC's in-house forecast where the center has it switched on, and the AFP weather product everywhere else.
...
- **NWAC's weather doesn't come from the AFP.** It is authored in-house and served by products-api as the Mountain Weather Forecast, with no `weather_product_id` pointer — AM/PM issuances are found by date. So the zone page's weather slot has two sources, and `getWeatherSourcesForForecast` in `weatherForForecast.ts` picks between them: the AFP product when `getWeatherForForecast` finds one, otherwise (for a center with NWAC weather switched on) the NWAC weather issuances narrowed to the zone (a day with none covering the zone counts as no weather, so there is no empty section or print option), for the viewed date on the dated archive route and the latest date with content on the live page. The Mountain Weather Archive tab reads the AFP product, so provisioning adds it only for `platforms.weather`, not for NWAC weather. Both take the one `weather` print section. The wire doesn't say which blocks a table covers (wind runs one day, snow level six or eight blocks), so `nwacWeatherFormat.ts` lets the data decide the columns.
