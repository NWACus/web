...
- **Unknown variables are headed with every word's initial** (`soil_temperature_b` → `STB`), as the native station pages do. The widget stopped at the first underscore, so `_a`, `_b` and `_c` were all "ST".
