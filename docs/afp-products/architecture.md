...
- **The station map ignores `widget_config.stations.alternate_zones`.** SAC, SNFAC and BTAC point it at a KML of forecaster-drawn station groupings, and the legacy widget uses those polygons for the zone filter, outlines and framing instead of the forecast zones. Native always uses the forecast-zone map layer, so for those centers the zone filter differs from (or, for SAC's single forecast zone, disappears next to) the widget's. `resolveStationMapSettings` surfaces the URL as `alternateZones`; keep the `stationMap` flag off for a center that has one until the KML path is built.
