...
- **The two info-exchange centers return `config: null`.** `EWYAIX` and `SOAIX` carry a `widget_config.danger_map` today, but their center-level `config` is null, `city` is null and `type` is `"other"` — none of which `avalancheCenterSchema` accepts yet (relaxing it is [#269 — Info-exchange homepage + platform support (land EWYAIX)](https://github.com/NWACus/web/issues/269)). Any code reading `widget_config.*` for map settings must tolerate its absence rather than assuming defaults exist; `resolveDangerMapSettings(undefined)` returns the dashboard's defaults for exactly this reason.
- **Information-exchange mode is a capability read, not a name test.** The legacy danger-map widget switches to its AIX mode when the center id ends in `AIX`. AvyWeb decides it from `platforms.*` via `isInformationExchange` (`!forecasts && obs`, the rule #269 records), which reaches the same two centers and also the observation-only ones the suffix misses. `stations` is deliberately not part of the test though both exchanges run them — nothing on the map is a weather station, and requiring it would un-classify an obs-only center that runs none. It is the only classification in the code: the legacy path's legend-height reservation reads it too rather than re-testing the suffix, accepting one first-paint shift if the rules ever diverge for a real center. **Nothing here is reachable yet** — no exchange can be a tenant (`ewyaix` is absent from `AVALANCHE_CENTERS`, and the schema rejects them per the bullet above), so the mode ships dark until #269. In that mode the map has no danger scale under it, and the exchange's own zones headline "View Observations" and link to `/observations` — **per zone**, so a neighboring forecast center's zone on an all-centers map keeps its rating and forecast link where the widget would pivot it too. The legend and the zone list's heading are still per map, as in the widget: an exchange's all-centers map (the shape [#1112 — Show surrounding-center forecasts on info-exchange sites](https://github.com/NWACus/web/issues/1112) is about) would paint neighbors' ratings with no scale to read them against, and that issue should decide the legend when it makes the case real.
- **The dashboard's danger-map height reaches no map.** The Mapbox widget build takes its height from `mapWidgetData.height` and otherwise falls back to its own 500px; it never reads `widget_config.danger_map.height`, which centers set anywhere from 350 to 500. Both paths therefore render at `DANGER_MAP_HEIGHT`, and honoring the configured value on the native path alone would shrink the map on the rollout flag flip for most centers. `resolveDangerMapSettings` still normalizes `height` for fidelity to the dashboard, but nothing consumes it — the same call already recorded for `saturation`.
