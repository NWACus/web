...
The toolbar's Table button swaps the map for the widget's current-conditions table (its `#/station-table` route, now `?view=table`; the old hash links still open it). Both views read the one response and the one filter state, so every center with the capability has the table whether or not it has station pages, and the map is unmounted while the table shows, so a reader who only wants the table never costs a Mapbox load. The table's rules are pure functions in `stationMap/table.ts`. Its threshold colors are the widget's English-unit numbers, applied only to a variable whose SnowObs unit is the one the rule was written in — the widget withheld them only when the reader chose metric, so a center whose default units were metric had millimeters colored as inches.
