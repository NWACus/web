...
Four behaviors look like divergences but match the widget. A station name opens that station's detail page, the native stand-in for the 24h modal the widget's table opened (#1341). A variable the widget has no short name for gets the widget's own initials, which space out only the first underscore — so `soil_temperature_a`, `_b` and `_c` are all "ST" and `precip_accum_three_hour` is "PA"; these are the headers forecasters know, and the long name is in each header's tooltip (the native station pages' `fallbackSensorLabel` spells out every word instead). Elevation is always in feet, because SnowObs doesn't convert it when asked for metric (the widget said "ft", and the map card now shows `'` in every unit system). And the Map/Table switch is a history entry, so Back returns to the previous view, as the widget's router link did.
