...
| Default built-in pages | Creates non-forecast built-in pages from the static `BUILT_IN_PAGES` list in `provisionTenant.ts` (see table below). Mountain Weather is only included if the center has a weather product: the AFP weather platform (`platforms.weather`) or NWAC's in-house Mountain Weather Forecast (`widget_config.mwf.enabled`). The Mountain Weather Archive needs the AFP platform. |
