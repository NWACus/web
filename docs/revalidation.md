...
- Returns published documents only. Pass `{ includeDrafts: true }` to see unpublished uses too — right for the "Where this is used" panel on a Shared Content document, where an editor is deciding whether a change is safe, and wrong for revalidation, which has no draft path to revalidate
