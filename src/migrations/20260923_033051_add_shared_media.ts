...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`shared_media\` (
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`shared_media_updated_at_idx\` ON \`shared_media\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`shared_media_created_at_idx\` ON \`shared_media\` (\`created_at\`);`,
  )
  await db.run(
    sql`CREATE UNIQUE INDEX \`shared_media_filename_idx\` ON \`shared_media\` (\`filename\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`shared_media_sizes_thumbnail_sizes_thumbnail_filename_idx\` ON \`shared_media\` (\`sizes_thumbnail_filename\`);`,
  )
  await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`shared_media\`;`)
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  await db.run(sql`CREATE TABLE \`__new_home_pages_blocks_media_block\` (
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_home_pages_blocks_media_block\`("_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "block_name") SELECT "_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "block_name" FROM \`home_pages_blocks_media_block\`;`,
  )
  await db.run(sql`DROP TABLE \`home_pages_blocks_media_block\`;`)
  await db.run(
    sql`ALTER TABLE \`__new_home_pages_blocks_media_block\` RENAME TO \`home_pages_blocks_media_block\`;`,
  )
  ...
  await db.run(sql`CREATE TABLE \`__new__home_pages_v_blocks_media_block\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new__home_pages_v_blocks_media_block\`("_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "_uuid", "block_name") SELECT "_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "_uuid", "block_name" FROM \`_home_pages_v_blocks_media_block\`;`,
  )
  await db.run(sql`DROP TABLE \`_home_pages_v_blocks_media_block\`;`)
  await db.run(
    sql`ALTER TABLE \`__new__home_pages_v_blocks_media_block\` RENAME TO \`_home_pages_v_blocks_media_block\`;`,
  )
  await db.run(
  ...
  await db.run(sql`CREATE TABLE \`__new_pages_blocks_media_block\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_pages_blocks_media_block\`("_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "block_name") SELECT "_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "block_name" FROM \`pages_blocks_media_block\`;`,
  )
  await db.run(sql`DROP TABLE \`pages_blocks_media_block\`;`)
  await db.run(
    sql`ALTER TABLE \`__new_pages_blocks_media_block\` RENAME TO \`pages_blocks_media_block\`;`,
  )
  await db.run(
  ...
  await db.run(sql`CREATE TABLE \`__new__pages_v_blocks_media_block\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new__pages_v_blocks_media_block\`("_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "_uuid", "block_name") SELECT "_order", "_parent_id", "_path", "id", "media_id", "caption", "background_color", "align_content", "image_size", "_uuid", "block_name" FROM \`_pages_v_blocks_media_block\`;`,
  )
  await db.run(sql`DROP TABLE \`_pages_v_blocks_media_block\`;`)
  await db.run(
    sql`ALTER TABLE \`__new__pages_v_blocks_media_block\` RENAME TO \`_pages_v_blocks_media_block\`;`,
  )
  await db.run(
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`station_pages_id\`) REFERENCES \`station_pages\`(\`id\`) ON UPDATE no action ON DELETE cascade,
