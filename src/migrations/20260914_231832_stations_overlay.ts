...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`station_groups\` (
  	...
  	\`slug\` text NOT NULL,
  	\`graph_axes_snow_depth_max\` numeric,
  	\`graph_axes_snowfall24_max\` numeric,
  	\`graph_axes_precip_max\` numeric,
  ...
  await db.run(sql`CREATE TABLE \`stations\` (
  	...
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`tenant_id\`) REFERENCES \`tenants\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`group_id\`) REFERENCES \`station_groups\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`stations_tenant_idx\` ON \`stations\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`stations_group_idx\` ON \`stations\` (\`group_id\`);`)
