...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`mwf_guidance_artifacts\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`mwf_guidance_artifacts_tenant_idx\` ON \`mwf_guidance_artifacts\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`mwf_guidance_artifacts_updated_at_idx\` ON \`mwf_guidance_artifacts\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`mwf_guidance_artifacts_created_at_idx\` ON \`mwf_guidance_artifacts\` (\`created_at\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`tenant_table_idx\` ON \`mwf_guidance_artifacts\` (\`tenant_id\`,\`table\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`mwf_guidance_artifacts_id\` integer REFERENCES mwf_guidance_artifacts(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_mwf_guidance_artifacts_id_idx\` ON \`payload_locked_documents_rels\` (\`mwf_guidance_artifacts_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`mwf_guidance_artifacts\`;`)
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`mwf_forecasts_id\`) REFERENCES \`mwf_forecasts\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "home_pages_id", "built_in_pages_id", "pages_id", "posts_id", "media_id", "galleries_id", "documents_id", "announcements_id", "sponsors_id", "tags_id", "events_id", "event_groups_id", "event_tags_id", "mwf_forecasts_id", "providers_id", "courses_id", "biographies_id", "teams_id", "users_id", "roles_id", "role_assignments_id", "global_roles_id", "global_role_assignments_id", "tenants_id", "navigations_id", "settings_id", "redirects_id", "forms_id", "form_submissions_id", "payload_mcp_api_keys_id") SELECT "id", "order", "parent_id", "path", "home_pages_id", "built_in_pages_id", "pages_id", "posts_id", "media_id", "galleries_id", "documents_id", "announcements_id", "sponsors_id", "tags_id", "events_id", "event_groups_id", "event_tags_id", "mwf_forecasts_id", "providers_id", "courses_id", "biographies_id", "teams_id", "users_id", "roles_id", "role_assignments_id", "global_roles_id", "global_role_assignments_id", "tenants_id", "navigations_id", "settings_id", "redirects_id", "forms_id", "form_submissions_id", "payload_mcp_api_keys_id" FROM \`payload_locked_documents_rels\`;`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_mwf_forecasts_id_idx\` ON \`payload_locked_documents_rels\` (\`mwf_forecasts_id\`);`)
