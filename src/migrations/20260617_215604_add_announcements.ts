...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`announcements\` (
  	...
  	\`display_interval\` numeric DEFAULT 3,
  	\`page_scope\` text DEFAULT 'all_pages',
  ...
  `)
  await db.run(sql`CREATE INDEX \`announcements_tenant_idx\` ON \`announcements\` (\`tenant_id\`);`)
  await db.run(
    sql`CREATE INDEX \`announcements_updated_at_idx\` ON \`announcements\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`announcements_created_at_idx\` ON \`announcements\` (\`created_at\`);`,
  )
  ...
  await db.run(sql`CREATE TABLE \`_announcements_v\` (
  	...
  	\`version_title\` text,
  	\`version_type\` text DEFAULT 'banner',
  	\`version_content\` text,
  	\`version_display_frequency\` text DEFAULT 'once',
  	\`version_display_interval\` numeric DEFAULT 3,
  	\`version_page_scope\` text DEFAULT 'all_pages',
  	\`version_device_target\` text DEFAULT 'all',
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`_announcements_v_parent_idx\` ON \`_announcements_v\` (\`parent_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_version_version_tenant_idx\` ON \`_announcements_v\` (\`version_tenant_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_version_version_updated_at_idx\` ON \`_announcements_v\` (\`version_updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_version_version_created_at_idx\` ON \`_announcements_v\` (\`version_created_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_version_version__status_idx\` ON \`_announcements_v\` (\`version__status\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_created_at_idx\` ON \`_announcements_v\` (\`created_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_updated_at_idx\` ON \`_announcements_v\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_announcements_v_latest_idx\` ON \`_announcements_v\` (\`latest\`);`,
  )
  await db.run(
