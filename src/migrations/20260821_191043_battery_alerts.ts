...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`station_alerts\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tenant_id\` integer NOT NULL,
  	\`stid\` text NOT NULL,
  	\`station_name\` text,
  	\`alerting\` integer DEFAULT true,
  ...
  `)
  await db.run(sql`CREATE INDEX \`station_alerts_tenant_idx\` ON \`station_alerts\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`station_alerts_stid_idx\` ON \`station_alerts\` (\`stid\`);`)
  await db.run(sql`CREATE INDEX \`station_alerts_alerting_idx\` ON \`station_alerts\` (\`alerting\`);`)
  await db.run(sql`CREATE INDEX \`station_alerts_updated_at_idx\` ON \`station_alerts\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`station_alerts_created_at_idx\` ON \`station_alerts\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`station_alerts_id\` integer REFERENCES station_alerts(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_station_alerts_id_idx\` ON \`payload_locked_documents_rels\` (\`station_alerts_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`station_alerts\`;`)
