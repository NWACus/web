...
/**
 * The stations a center tracks in SnowObs, for the picker on a station page.
 * A proxy so the admin never handles the token, cached an hour server-side.
...
export const trackedStations: PayloadHandler = async (req) => {
  ...
  try {
    const stations = await fetchTrackedStations(await resolveSnowObsToken(center))
