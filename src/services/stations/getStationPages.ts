...
async function loadStationPages(center: string): Promise<StationPage[]> {
  ...
  const [{ docs: groups }, { docs: stations }] = await Promise.all([
    payload.find({
    ...
    payload.find({
      collection: 'stations',
      where: { and: [{ 'tenant.slug': { equals: center } }, { group: { exists: true } }] },
      depth: 0,
      limit: 0,
      pagination: false,
    }),
  ...
  return assembleStationPages(groups, stations)
}
...
export async function allStationPageParams(): Promise<{ center: string; station: string }[]> {
  ...
  return docs.flatMap((group) => {
    ...
    return [{ center: tenant.slug, station: group.slug }]
  })
}
