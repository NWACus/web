/**
 * Zod schemas for the products-api public NWAC weather reads,
 * `/v3/public/nwac-weather/forecast` and `/forecasts`. NWAC forecasts mountain weather in-house,
 * so this product has no v2 shape: products-api is its only backend.
 *
 * The payload is already render-ready — every slot carries its resolved calendar date and the
 * rows are flat `{ zone | point, period | block, ...values }` — so the schema describes it as
 * sent and the mapper only re-keys the rows for lookup.
 */
import { z } from 'zod'
...
export const nwacWeatherPeriodSchema = z.object({
  ...
  /** `YYYY-MM-DD`, resolved server-side from the service date. */
  date: z.string(),
