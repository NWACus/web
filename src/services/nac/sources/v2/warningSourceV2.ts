...
import { fetchWarning, fetchWarningFresh } from '../../nac'
...
export const warningSourceV2: WarningSource = {
  ...
  async getWarningFresh(centerId: string, zoneId: number): Promise<WarningProduct | null> {
    const wire = await fetchWarningFresh(centerId, zoneId)
    return wire === null ? null : mapV2Warning(wire)
  },
}
