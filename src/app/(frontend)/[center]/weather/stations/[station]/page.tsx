...
async function loadDataloggers(
  ...
  const meta = await fetchStationTimeseries(page.stids, { windowHours: 1, source })
...
async function csvTabView({ page, source }: TabContext): Promise<TabView> {
  return {
    table: null,
    tabContent: (
      <>
        ...
        <StationCsvForm
          slug={page.slug}
          dataloggers={await loadDataloggers(page, source)}
          years={csvYears()}
        />
      </>
    ),
...
function graphsTabView({ page, pages }: TabContext): TabView {
  return {
    ...
    tabContent: (
      <StationGraphs
        stids={page.stids}
        presets={applyGraphAxes(STATION_GRAPH_PRESETS, page.graphAxes)}
...
async function tableTabView({ page, source, periodParam }: TabContext): Promise<TabView> {
  const period = resolveTablePeriod(periodParam)
  const response = await fetchStationTimeseries(page.stids, {
    revalidate,
...
function defaultTabKey(page: StationPage): string {
...
export default async function Page({ params, searchParams }: Args) {
  ...
  const view = await resolveTabView(
    { page, pages: toPageSummaries(pages), source: center },
    rangeParam,
    periodParam,
  )
