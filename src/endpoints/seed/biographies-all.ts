import type { Biography, Tenant } from '@/payload-types'
import { RequiredDataFromCollectionSlug } from 'payload'

export const biographies = (
  tenantId: number,
): Record<string, RequiredDataFromCollectionSlug<'biographies'>[]> => {
  return {
    nwac: [
      {
        tenant: tenantId,
...
): Record<string, { alt: string; filename: string; tenant: Tenant }[]> => {
  return {
    nwac: [
      {
        alt: 'Lee Lazzara',
